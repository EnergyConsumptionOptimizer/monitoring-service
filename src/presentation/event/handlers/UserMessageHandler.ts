...
import { userDeletedPayloadSchema } from "@presentation/event/contracts/UserEventPayloads";
...
export class UserMessageHandler {
  ...
  async handle(raw: string): Promise<void> {
    ...
    await tracer.startActiveSpan(
      `UserMessageHandler.handle.${message.eventType}`,
      {
        attributes: {
          eventId: message.eventId,
          correlationId,
  ...
  async #getHandlerOrDiscard(
    ...
    switch (message.eventType) {
      case "UserDeletedEvent":
        handlerFunction = this.#handleUserDeleted(message.payload);
        break;
    }
  ...
  #handleUserDeleted(
    ...
    return () =>
      this.#measurementMaintenanceService.removeHouseholdUserTagFromMeasurements(
